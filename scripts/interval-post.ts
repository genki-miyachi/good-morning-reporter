/**
 * Interval post script
 */

import { config } from 'dotenv';
import fs from 'node:fs/promises';
import { postResult } from './discord-client.js';
import { splitByLimit } from './post.js';
...
function getDateInTimezoneAt(now: Date, timeStr: string, timeZone: string): Date {
...
async function main(): Promise<void> {
  ...
  const data = JSON.parse(raw) as { messages: string[] };
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (dryRun) {
      ...
      for (const chunk of chunks) {
        await postResult(channelId, chunk, token);
      }
    }
