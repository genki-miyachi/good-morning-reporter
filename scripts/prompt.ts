...
function getMessageContent(msg: DiscordMessage): string {
...
function formatMsToJst(ms: number): string {
  const d = new Date(ms);
...
function getMessageTimestampJst(msg: DiscordMessage): string {
...
function analyzeMessages(messages: DiscordMessage[]) {
...
function serializeRecentBotPosts(recentBotPosts: HistoryEntry[] = []): string {
  ...
  const sanitized = recentBotPosts.map((p) => {
    ...
    const mentionUserIds = Array.from(trimmed.matchAll(/<@(?<id>\d+)>/g))
      .map((m) => m.groups?.id)
